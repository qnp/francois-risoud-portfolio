...
export default {
  ...
  methods: {
    ...
    setScroll: function() {
      ...
      import(/* webpackChunkName: "iscroll-probe" */'iscroll/build/iscroll-probe.js').then(function(IScroll) {

        self.myScroll = new IScroll('#iscroll', {
          probeType: 2,
          scrollX: false,
          scrollY: true,
          click: true,
  ...
  watch: {
    show: function(newVal, oldVal) {
      if (newVal) {
        this.setScroll();
        setTimeout(() => { this.$emit('on'); }, 600);
      } else {
        this.unsetScroll();
        setTimeout(() => { this.$emit('off'); }, 400);
      }
    }
  }
