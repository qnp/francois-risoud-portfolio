...
class MarchingSquare {
  ...
  constructor(settings) {
    ...
    var errorStr = "[MarchingSquare] constructor must accept a valid 'settings' object: {\n";
    ...
    if (settings === undefined || !Object.keys(settings).length) {
      ...
      Object.entries(model).forEach(function(entry) {
        if (settings[entry[0]] === undefined) {
          ...
          errorStr += "=> typeof '" + entry[0] + "' was found to be '" + (typeof settings[entry[0]]) + "'\n";
          throw new Error(errorStr);
        }
      });
    }
  ...
  finePosOpt(p, l, val1, val2) {
    return (l * (this.threshold - val1*(p+1) + val2*p) / (val2 - val1))|0;
  }
  ...
  loop() {
    ...
    while (i--) {
      ...
      while (j--) {
        ...
        if (found && (x1 !== x2 || y1 !== y2)) this.points = this.points.concat([{x: x1-m*w, y: y1-m*h, cas: cas}, {x: x2-m*w, y: y2-m*h, cas: cas}]);
        if (ambiguous && (x3 !== x4 || y3 !== y4)) this.points = this.points.concat([{x: x3-m*w, y: y3-m*h, cas: cas}, {x: x4-m*w, y: y4-m*h, cas: cas}]);
